...
function pdfHtmlKayit(veri, meta) {
  ...
  var satirlar = [
    ...
    [ikiDilliEtiket(ETIKET_KAYIT.goruntuSosyalIzni, dil), evetHayirGoster(!!veri.goruntuSosyalIzni, dil)],
  ...
  if (saglik.var) {
    ...
    var saglikMetin = meta.saglikGizle
      ? (dil === "fr" ? "Conservée uniquement dans le registre d’inscription ; non reprise dans ce document."
         : dil === "en" ? "Kept only in the registration register; not included in this document."
         : "Yalnız kayıt defterinde tutulur; bu belgeye yazılmamıştır.")
      : kacis(saglik.not || "");
...
function kayitPostIsleV2(v) {
  try {
    ...
    try {
      ...
      dosya = klasor.createFile(saglik.var ? htmlPdfUret(pdfHtmlKayit(v, Object.assign({}, meta, { saglikGizle: true })), dosyaAdi) : blob);
