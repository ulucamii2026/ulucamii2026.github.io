/* v26 — Cami seçimi, camiye göre şahitler ve otomatik tam PDF paketi. */
/**
 * Marche-en-Famenne Ulu Camii — Ortak Apps Script Alıcı (SÜRÜM 26)
...
function doGet(e) {
  ...
  return json({ ok: true, servis: "ulucamii-alici", surum: SURUM,
    ihtidaPaketHazir: typeof IhtidaPdf !== "undefined" && ["25", "26"].indexOf(paketSurumu) >= 0,
...
function doPost(e) {
  try {
    ...
    if (v.tur === "ihtida") {
      if (typeof IhtidaPdf === "undefined" || ["25", "26"].indexOf(PropertiesService.getScriptProperties().getProperty("IHTIDA_PAKET_KURULU")) < 0) return json({ ok: false, hata: "ihtida-paket-hazir-degil" });
