/* v29 — 13 Eyl 2026: Kayıt kimlik teslimi, Drive görselleri ve dürüst e-posta sonucu.
...
/**
 * Marche-en-Famenne Ulu Camii — Ortak Apps Script Alıcı (SÜRÜM 29)
...
var SURUM = 29;
var DIN_GOREVLISI_WHATSAPP = "32471794682";
...
/* ===================================================================
   ...
   (secenekler) veya (alici, konu, govde).
   =================================================================== */
var BREVO_GONDEREN = { name: EPOSTA.ad, email: "info@ulucamii.be" };
...
function brevoGonder(s, anahtar) {
  var govde = { sender: BREVO_GONDEREN, to: brevoAlicilar(s.to), subject: String(s.subject || "") };
...
function epostaGonder(a, konu, govde) {
  var s = (typeof a === "string") ? { to: a, subject: konu, body: govde } : (a || {});
  var anahtar = brevoAnahtari();
  if (anahtar) {
    try { return brevoGonder(s, anahtar); }
...
function brevoSina() {
  ...
  var sonuc = epostaGonder({
    to: "info@ulucamii.be", subject: "Brevo deneme - surum " + SURUM, body: "Duz metin govde.",
    htmlBody: mailHtml("<p>Brevo üzerinden gönderildi (sürüm " + SURUM + ").</p>"),
    attachments: [blob], name: EPOSTA.ad, replyTo: EPOSTA.yanit
  });
...
function ihtidaKopyaGonderV2(blob, ref, adSoyad, eposta) {
  ...
  try {
    epostaGonder({ to: adres, subject: konu, body: govde, htmlBody: mailHtml(govde), attachments: [blob], name: EPOSTA.ad, replyTo: EPOSTA.yanit });
  } catch (hata) {
