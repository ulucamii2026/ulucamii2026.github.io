...
function panelListeIsle(e) {
  try {
    ...
    return json({
      ok: true, surum: 9, zaman: new Date().toISOString(),
...
function kayitPostIsle(v) {
  try {
    ...
    try {
      ...
      if (guncelleme) eskiSatiriIsaretle(sayfa, eskiRef, ref);
    } finally {
      kilit.releaseLock();
    }
    ...
    try {
      ...
      for (var si = 0; si < 2; si++) {
        ...
        if (s.imza) {
          ...
          } catch (sHata) { console.error("sahit imzasi kaydedilemedi: " + sHata); }
        }
      ...
      if (v.vesikalikBase64) {
        ...
        } catch (vHata) { console.error("vesikalik kaydedilemedi: " + vHata); }
      }
      if (v.basvuranImzaBase64) {
        try {
          var iBlob = veriUrlBlob(v.basvuranImzaBase64, ref + " - imza.png");
          if (iBlob) klasor.createFile(iBlob);
        } catch (iHata) { console.error("basvuran imzasi kaydedilemedi: " + iHata); }
      }
