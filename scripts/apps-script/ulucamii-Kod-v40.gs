 ...
 *   Vesikalık fotoğraf, kimlik belgesinin ön/arka yüzü ve ekranda çizilen imza,
 *   İhtida Belgesi'nin (EK-9) düzenlenmesi ve resmî dosyanın oluşturulması içindir; belge ve
 *   ekleri hem camimizde hem T.C. Brüksel Büyükelçiliği Sosyal İşler Müşavirliği'nde saklanır.
 *   Dosyalar İHTİDA klasörüne "<ref> - vesikalik.jpg | kimlik-on.jpg | kimlik-arka.jpg | imza.png"
 *   adlarıyla yazılır — DEFTERE YENİ SÜTUN EKLENMEZ. Panel bunları ?islem=belge ile geri okur,
 *   ?islem=ihtida-gorsel-sil&ref=… ile TEK BAŞVURUNUN görselleri tek tek silinir (toplu silme yok).
 *   Kimlik NUMARASI hâlâ hiçbir yerde toplanmaz; KAYIT (çocuk) formu eski kuralda kalır.
 *
 ...
 *     (İHTİDA formu için bu madde 8 Eylül 2026'da geri alındı — yukarıya bakınız.)
 ...
 *   - v21 (4 Eylül 2026 akşam): ?islem=pdf&id=<Drive kimliği>&anahtar=… — panelin «Form PDF»
 *     düğmesi. Drive bağlantısı yalnız dosya sahibine açıktı; başka hesapla girmiş yönetici
 *     «erişim isteyin» sayfasına düşüyordu. Dosyalar paylaşıma açılmadı; panel dosyayı bu uçtan
 *     base64 alır (yalnız kayıt/ihtida klasörlerindeki, çöpte olmayan PDF'ler). Bkz. panelPdfIsle().
 *
...
function doGet(e) {
  // 13 Eyl 2026: Kayıt kimlik uçlarını yetkili panel yardımcılarına yönlendirir.
  if (e && e.parameter) {
    ...
    if (e.parameter.islem === "kayit-belge") return kayitBelgeIsle(e);
    if (e.parameter.islem === "kayit-gorsel-sil") return kayitGorselSilIsle(e);
    if (e.parameter.islem === "pdf") return panelPdfIsle(e);
...
/* 13 Eyl 2026: Kayıt JSON'u base64 ve alanlarıyla toplam 4 MiB UTF-8 baytı olabilir.
...
function doPost(e) {
  ...
  try {
    ...
    if (govde.length > 15 * 1024 * 1024) return json({ ok: false, hata: "cok-buyuk" });
...
function pdfUst(baslikTr, baslikDil, dil, ref, zaman, ekSatirHtml) {
  ...
  var kunye = [KIMLIK.kurumlar.cami.ad.tr, KIMLIK.hukuki.resmiAd + " " + KIMLIK.hukuki.hukukiForm,
    "KBO " + KIMLIK.hukuki.kbo, KIMLIK.iletisim.adres.tekSatir, KIMLIK.iletisim.eposta.genel].join(" — ");
  return '<header class="ust"><div class="kunye">' + kacis(kunye) + '</div>'
...
function pdfHtmlKayit(veri, meta) {
  ...
  var imzaHucre = imzaGorsel
    ...
    : '<div class="imza-cizgi"></div><div class="imza-etiket">' + E("imza") + " — " + kacis(onay.elektronikImza) + "</div>";
  ...
  var imzaNotuTr = imzaGorsel
    ...
    : "; çizilmiş veli imzası kayıtlı değildir; boş imza alanı veli tarafından kalemle imzalanmalıdır.";
  var imzaNotuFr = imzaGorsel
    ...
    : " ; aucune signature tracée du parent n’est enregistrée ; la case vide doit être signée au stylo par le parent.";
  ...
  return "<!DOCTYPE html><html lang=\"" + kacis(dil) + "\"><head><meta charset=\"utf-8\"><title>" + kacis(ref)
    ...
    + sayfa1
    + sayfa2
    + kayitPdfKimlikEki(veri, ref)
    + altBilgi
    + "</body></html>";
}

/** Kayıt dosyasının özel kimlik eki; yalnız yüklenen görseller, haricî URL yok. */
function kayitPdfKimlikEki(veri, ref) {
  var kimlik = veri.kimlik || {};
  var gecerli = function (v) { return typeof v === "string" && /^data:image\/(?:png|jpeg|webp);base64,[A-Za-z0-9+\/=]+$/.test(v); };
  if (!gecerli(kimlik.on) && !gecerli(kimlik.arka)) return "";
  var html = '<section class="kimlik-ek" style="page-break-before:always"><h1>Öğrencinin kimlik belgesi / Pièce d’identité de l’élève</h1><p>Ref ' + kacis(ref) + '</p>';
  [["on", "Ön yüz / Recto"], ["arka", "Arka yüz / Verso"]].forEach(function (alan) {
    if (gecerli(kimlik[alan[0]])) html += '<div style="page-break-inside:avoid;margin:16px 0"><h2>' + alan[1] + '</h2><img class="kimlik-gorsel" style="display:block;max-width:100%;max-height:310px;margin:8px auto" src="' + kimlik[alan[0]] + '" alt=""></div>';
  });
  return html + '<p>Yalnız kurs kayıt dosyası içindir. / Réservé au dossier d’inscription au cours.</p></section>';
}
...
function pdfHtmlIhtida(veri, meta) {
  ...
  var beyanBlok = '<section class="blok"><h2>Beyan / Déclaration</h2>'
    ...
    + "<p>Belge görselleri açık rızayla başvuruda alınır. Şahitlik ve imzalar törende teyit edilir. "
    + "Dosya Müşavirliğe gönderilir; imzalanan belge seçilen teslim yerine postalanır. / "
    + "Les documents sont recueillis avec votre consentement. Les témoins et les signatures sont confirmés lors de la cérémonie. "
    + "Après la signature du Conseiller, l’attestation est envoyée au lieu choisi.</p>"
    + "</section>";
...
function epostaGecerli(s) {
  // 13 Eyl 2026: Ön yüzle aynı, en az iki harfli alan adı sonunu zorunlu kılar.
...
function kayitDogrulaV2(v) {
  // 13 Eyl 2026: v2/v3, yaş, tam adres ve kimlik rızasını ön yüzle eşler.
  if (!v || typeof v !== "object") return h("bos-istek");
  if (v.sir !== AYAR2.ortakSir) return h("yetkisiz");
  if ([2, 3].indexOf(v.formSurumu) === -1) return h("form-surumu-gecersiz");
  ...
  var yasSonuc = yasDogrula(o.dogumTarihi, 4, 18);
  ...
  if (!metinDolu(veli.postaKodu)) return h("veli-posta-kodu-eksik");
  if (!uzunlukTamam(veli.postaKodu, 12)) return h("veli-posta-kodu-uzun");
  if (!metinDolu(veli.sehir)) return h("veli-sehir-eksik");
  if (!uzunlukTamam(veli.sehir, 80)) return h("veli-sehir-uzun");
  ...
  // 15 Eyl 2026: yeni kayıtlar çizilen veli imzası olmadan kabul edilmez; eski istemciler de geçemez.
...
function kayitKimlikDogrula(v) {
  // 15 Eyl 2026: kimlik ön yüzü (pasaportta kimlik sayfası) ve rıza zorunlu.
  var k = v.kimlik;
  if (!k || typeof k !== "object" || Array.isArray(k) || k.yol !== "yukle") return h("kimlik-yol-gecersiz");
  if (!v.onay || v.onay.kimlikRiza !== true) return h("kimlikRiza");
  if (typeof k.on !== "string" || !/^data:image\/(?:png|jpeg|webp);base64,[A-Za-z0-9+\/=]+$/.test(k.on) || !gorselGecerli(k.on, 1.5)) return h("kimlik-on-gecersiz");
  if (k.arka !== undefined && k.arka !== "" && (typeof k.arka !== "string" || !/^data:image\/(?:png|jpeg|webp);base64,[A-Za-z0-9+\/=]+$/.test(k.arka) || !gorselGecerli(k.arka, 1.5))) return h("kimlik-arka-gecersiz");
  return { tamam: true };
}

function kayitKimlikOzeti(kimlik) {
  // 13 Eyl 2026: Defter/e-posta yalnız sabit teslim özetini taşır, görsel taşımaz.
  var k = kimlik || { yol: "belirsiz" };
  if (k.yol === "yukle") return k.on ? (k.arka ? "yüklendi (ön+arka)" : "yüklendi (ön)") : "";
  return { eposta: "e-posta ile gelecek", whatsapp: "WhatsApp ile gelecek", elden: "elden gösterilecek" }[k.yol] || "";
}
...
function ihtidaDogrulaV2(v) {
  ...
  if (!metinDolu(b.adres) || !uzunlukTamam(b.adres, 400)) return h("basvuran-adres-eksik");
  // Tam posta adresi yalnız tarayıcıya bırakılmaz; eksik parça sunucuda da reddedilir.
  if (!metinDolu(b.adresSokak) || !uzunlukTamam(b.adresSokak, 200) ||
      !metinDolu(b.postaKodu) || !uzunlukTamam(b.postaKodu, 20) ||
      !metinDolu(b.sehir) || !uzunlukTamam(b.sehir, 80) ||
      !metinDolu(b.ulke) || !uzunlukTamam(b.ulke, 80)) return h("basvuran-adres-eksik");
  var tamAdres = [b.adresSokak, b.postaKodu, b.sehir, b.ulke].map(function(p) { return String(p).trim(); }).join(", ");
  if (b.adres !== tamAdres) return h("basvuran-adres-eksik");
  ...
  var cami = ihtidaCamiCoz(v.cami);
  if (!cami) return h("cami-gecersiz");

  // Eski başvurularla uyum: yeni alanlar birlikte bulunmalı; eksikler uydurulmaz.
  if (v.teslimat !== undefined || v.sahitSecimi !== undefined) {
    if (!v.teslimat || ["cami", "adres"].indexOf(v.teslimat.yontem) === -1) return h("teslimat-gecersiz");
    if (["cami", "kendi"].indexOf(v.sahitSecimi) === -1) return h("sahit-secimi-gecersiz");
    if (!Array.isArray(v.sahitler) || v.sahitler.length > 2) return h("sahit-sayisi-gecersiz");
    if (v.sahitSecimi === "kendi" && (!v.sahitler[0] || !metinDolu(v.sahitler[0].ad))) return h("sahit-adi-eksik");
    if (v.sahitSecimi === "cami" && v.sahitler.length) return h("sahit-secimi-tutarsiz");
  }
  ...
  /* v23: belge görselleri ve imza. Gövde büyüdüğü için sınırlar burada da denetlenir —
     istemcideki küçültmeye güvenilmez (istemci doğrulaması bağlayıcı değildir). */
  if (["kimlik", "pasaport"].indexOf(v.belgeTuru) === -1) return h("belge-turu-gecersiz");
  if (typeof v.imzaYok !== "boolean") return h("imza-yok-gecersiz");
  var g = v.gorseller || {};
  if (!gorselGecerli(g.vesikalik, 3)) return h("vesikalik-gecersiz");
  if (!gorselGecerli(g.kimlikOn, 3)) return h("kimlik-on-gecersiz");
  if (v.belgeTuru === "kimlik" && !gorselGecerli(g.kimlikArka, 3)) return h("kimlik-arka-gecersiz");
  if (v.belgeTuru === "pasaport" && g.kimlikArka) return h("kimlik-arka-fazla");
  if (!v.imzaYok && !gorselGecerli(g.imza, 1)) return h("imza-gecersiz");
  if (v.imzaYok && g.imza) return h("imza-fazla");
  if (onay.gorselRiza !== true) return h("onay-gorsel-riza-eksik");

  if (onay.acikRiza !== true) return h("onay-acik-riza-eksik");
  if (onay.ek10 !== true) return h("onay-ek10-eksik");
  /* V27 tarayıcısı sürümü açıkça gönderir. V26 istemcisi yayındayken gelen
     sürümsüz gövde ise eski EK-10 sayılır; yeni rıza uydurulmaz. Böylece
     rollout sırası başvuruyu reddetmez, paketleyici de v1 şablonu seçer. */
  var yeniEk10 = onay.ek10Surumu === "2026-09-09";
  if (onay.ek10Surumu !== undefined && !yeniEk10) return h("onay-ek10-surum-gecersiz");
  if (yeniEk10 && !v.imzaYok && onay.imzaAktarimIzni !== true) return h("onay-imza-aktarim-izni-eksik");
  if (yeniEk10 && v.imzaYok && onay.imzaAktarimIzni !== false) return h("onay-imza-aktarim-izni-gecersiz");
  if (onay.gizlilik !== true) return h("onay-gizlilik-eksik");
...
/** Yeni kayıtlar katalog kimliğiyle sunucuda kanonikleştirilir. Cami bilgisi olmayan
 * eski satırlar Marche varsayılır; bilinmeyen ya da yarım başka cami asla Marche'a düşmez. */
function ihtidaCamiCoz(girdi) {
  if (girdi === undefined || girdi === null) return { id: "ulucamii-marche", ad: "Ulu Camii", sehir: "Marche-en-Famenne", postaKodu: "6900", adres: "Thier des Corbeaux 14", kurum: "BDV" };
  if (typeof IhtidaPdf === "undefined" || typeof IhtidaPdf.camiCoz !== "function") return null;
  try {
    var cami = IhtidaPdf.camiCoz(girdi);
    if (!cami || !metinDolu(cami.id) || !metinDolu(cami.ad) || !metinDolu(cami.sehir) || !metinDolu(cami.postaKodu) || !metinDolu(cami.adres)) return null;
    return cami;
  } catch (_) { return null; }
}
...
var BASLIKLAR2 = [
  ...
  "PDF bağlantısı", "Durum", "Gönderim anahtarı", "Sosyal medya izni", "Kimlik belgesi" // 13 Eyl 2026: yalnız sona eklenir.
];
...
function kayitDurumNotuEkle(sayfa, ref, not) {
  // 13 Eyl 2026: E-posta sonucu kimlik hatasını ezmeden kalıcı Durum hücresine eklenir.
  var kilit = LockService.getScriptLock();
  kilit.waitLock(30000);
  try {
    var satir = satirBulGenel(sayfa, ref, SUTUN2.referans);
    if (satir < 2) return;
...
function kayitPostIsleV2(v) {
  ...
  try {
    ...
    var kilit = LockService.getScriptLock();
    kilit.waitLock(30000);
    var ref, dosya, blob, okulHucre, kimlikOzeti = "", kimlikHatalari = [];
    ...
    try {
      epostaGonder({
        ...
        body: [
          ...
          "Referans      : " + ref,
          "Kimlik belgesi: " + (kimlikOzeti || "—") + (kimlikHatalari.length ? " | kimlik-kayit-hatasi" : ""),
          "Öğrenci       : " + adSoyad,
        ...
        attachments: [blob],
        kurum: "kurs", name: KIMLIK.kurumlar.kurs.gonderenAdi,
      ...
      try {
        kayitDurumNotuEkle(sayfaV2, ref, "e-posta gönderilemedi");
      } catch (_) {}
    }

    var kopyaGitti = false;
    try { kopyaGitti = kopyaGonderV2(blob, ref, adSoyad, [veli.eposta], veli.iletisimDili, kimlik, v.dil, kimlikHatalari) === true; }
    catch (kopyaHatasi) { console.error("kopya gonderilemedi: " + kopyaHatasi); }
    try { kayitDurumNotuEkle(sayfaV2, ref, kopyaGitti ? "veli-kopyasi-gonderildi" : "veli-kopyasi-gonderilemedi"); }
    catch (durumHatasi) { console.error("Kopya sonucu deftere yazılamadı"); }
    if (anahtar) kayitV2AnahtarKaydet(anahtar, ref, kopyaGitti);

    return json({ ok: true, ref: ref, tekrar: false, kopyaGitti: kopyaGitti });
  } catch (hata) {
    console.error(hata);
    return json({ ok: false, hata: "sunucu-hatasi", ayrinti: String(hata).slice(0, 200) });
  }
...
/* --- E-POSTA KİMLİĞİ v30: bütün ad/adresler üretilen KIMLIK'ten --- */
var EPOSTA = { yanit: KIMLIK.iletisim.eposta.yanit, ad: KIMLIK.kurumlar.cami.gonderenAdi };
function epostaKimligi(kurum) {
  if (kurum === undefined) kurum = "cami";
  if (["kurs", "cami"].indexOf(kurum) < 0) throw new Error("veli-eposta-kurum");
  return { name: KIMLIK.kurumlar[kurum].gonderenAdi, email: KIMLIK.kurumlar[kurum].epostaAdresi };
}

/* ===================================================================
   E-POSTA GÖNDERİMİ — v22: Brevo HTTP API üzerinden info@ulucamii.be
   -------------------------------------------------------------------
   MailApp betiği çalıştıran Gmail hesabından gönderir ("Kimden" satırında
   ulucamii2026@gmail.com görünür); Gmail'in "Send mail as" yolu üçüncü taraf
   adresler için Ocak 2027'de kalkıyor. Bu yüzden gönderim Brevo (Sendinblue SAS)
   API'siyle yapılır: alan adı ulucamii.be Brevo'da DKIM ile doğrulanmıştır.
   API anahtarı Script Properties → BREVO_API_KEY (koda yazılmaz; kod public depoda).
   Anahtar yoksa ya da Brevo hata verirse MailApp'e düşülür: onay hiç kaybolmaz.
   epostaGonder() MailApp.sendEmail ile aynı imzayı kabul eder:
   (secenekler: {kurum?, ...}) veya (alici, konu, govde, kurum?).
   =================================================================== */
var BREVO_GONDEREN = epostaKimligi("cami");
var BREVO_UC = "https://api.brevo.com/v3/smtp/email";

function brevoAnahtari() {
  try {
    var k = PropertiesService.getScriptProperties().getProperty("BREVO_API_KEY");
    return k ? String(k).trim() : "";
  } catch (hata) { return ""; }
}

function brevoAlicilar(liste) {
  return String(liste || "").split(",").map(function (x) { return x.trim(); })
    .filter(function (x) { return epostaGecerli(x); }).map(function (x) { return { email: x }; });
}

function brevoGonder(s, anahtar, kurum) {
  var govde = { sender: epostaKimligi(kurum === undefined ? s.kurum : kurum), to: brevoAlicilar(s.to), subject: String(s.subject || "") };
  if (!govde.to.length) throw new Error("gecerli alici yok");
  if (s.htmlBody) govde.htmlContent = String(s.htmlBody);
  if (s.body) govde.textContent = String(s.body);
  if (!govde.htmlContent && !govde.textContent) govde.textContent = " ";
  if (s.replyTo && epostaGecerli(s.replyTo)) govde.replyTo = { email: String(s.replyTo).trim() };
  if (s.cc) govde.cc = brevoAlicilar(s.cc);
  if (s.attachments && s.attachments.length) {
    govde.attachment = s.attachments.map(function (b, i) {
      return { name: (b.getName && b.getName()) || ("ek-" + (i + 1)), content: Utilities.base64Encode(b.getBytes()) };
    });
  }
  var yanit = UrlFetchApp.fetch(BREVO_UC, {
    method: "post", contentType: "application/json", headers: { "api-key": anahtar, accept: "application/json" },
    payload: JSON.stringify(govde), muteHttpExceptions: true
  });
  var kod = yanit.getResponseCode();
  if (kod < 200 || kod >= 300) throw new Error("Brevo HTTP " + kod + ": " + String(yanit.getContentText()).slice(0, 300));
  return { yol: "brevo", kod: kod, cevap: String(yanit.getContentText()).slice(0, 200) };
}

function epostaGonder(a, konu, govde, kurum) {
  var s = (typeof a === "string") ? { to: a, subject: konu, body: govde } : Object.assign({}, a || {});
  kurum = s.kurum === undefined ? kurum : s.kurum;
...
/** Düzenleyiciden elle çalıştırılır: Brevo yolunu info@'ya küçük ekli bir denemeyle sınar. */
function brevoSina() {
  var blob = Utilities.newBlob("Brevo deneme eki - " + new Date().toISOString(), "text/plain", "deneme.txt");
  var sonuc = epostaGonder({
    to: KIMLIK.iletisim.eposta.genel, subject: "Brevo denemesi — sürüm " + SURUM, body: "Brevo üzerinden gönderildi (sürüm " + SURUM + ").",
    htmlBody: mailHtml("Brevo üzerinden gönderildi (sürüm " + SURUM + ").", "tr", "E-posta gönderim denemesi", "cami"),
    attachments: [blob], kurum: "cami", name: EPOSTA.ad, replyTo: EPOSTA.yanit
  });
...
/* Eski çağrı adı korunur; bağımsız HTML/tasarım üretmez. Dil açıkça verilmelidir. */
function mailHtml(govde, dil, baslik, kurum) {
  return veliEpostaDuzMetin(govde, dil, baslik, {kurum: kurum === undefined ? "cami" : kurum});
}
...
function kopyaGonderV2(blob, ref, adSoyad, adresler, dil, kimlik, formDili, kimlikHatalari) {
  ...
  if (!gecerli.length) return false;
  ...
  var govde = (dil === "tr" ? [
    ...
    .concat([
      ...
      "",
      "Bilgilerinizde bir düzeltme gerekirse " + KIMLIK.iletisim.eposta.genel + " adresine yazınız; " + ref + " numarasını belirtiniz.",
      ""
    ]) : [
      "Bonjour,", "",
    ...
    .concat([
      ...
      "",
      "Pour corriger une information, écrivez à " + KIMLIK.iletisim.eposta.genel + " en indiquant le numéro " + ref + ".",
      ""
    ])).concat(dil === "fr" ? [
  ...
  govde += "\n\n" + KIMLIK.yazisma.kapanis[dil].genel;
  var ekler = [blob];
  ...
  gecerli.forEach(function (adres) {
    try {
      epostaGonder({
        to: adres, subject: konu, body: govde + "\n\n" + KIMLIK.yazisma.imza.kurs[dil].join("\n"), htmlBody: veliEpostaDuzMetin(govde, dil, konu, {kurum: "kurs"}),
        attachments: ekler, kurum: "kurs", name: KIMLIK.kurumlar.kurs.gonderenAdi, replyTo: EPOSTA.yanit
      });
    } catch (hata) {
      hepsiGitti = false;
...
var BASLIKLAR2_IHTIDA = [
  ...
  "Fotoğraf izni", "Şahit 1", "Şahit 2", "Açık rıza", "Elektronik beyan", "Form dili", "PDF bağlantısı", "Durum", "Gönderim anahtarı", "Belge teslim yeri", "Şahit seçimi", "EK-10 rızası",
  "Adres sokak", "Posta kodu", "Şehir", "Ülke", "Kimlik belgesi türü", "Tam paket PDF", "Paket durumu", "E-posta durumu",
  "Cami kimliği", "Başvuru camisi", "Cami şehri", "Cami posta kodu", "Cami adresi", "Cami kurumu",
  "EK-10 sürümü", "İmza aktarım izni"
];
...
/* ---------- v23: İhtida belge görselleri ---------- */

/** İhtida klasöründeki dosya adlarının son ekleri. Sıra panelin beklediği sıradır. */
var IHTIDA_GORSEL_ADLARI = [
  { anahtar: "vesikalik", sonEk: " - vesikalik.jpg" },
  { anahtar: "kimlikOn", sonEk: " - kimlik-on.jpg" },
  { anahtar: "kimlikArka", sonEk: " - kimlik-arka.jpg" },
  { anahtar: "imza", sonEk: " - imza.png" }
];
...
/** Gelen görselleri İhtida klasörüne yazar; kaydedilemeyenlerin adlarını döndürür.
 *  Görsel düşse bile BAŞVURU KAYDEDİLİR — kişi formu bir daha doldurmasın. */
function ihtidaGorselleriKaydet(klasor, ref, gorseller) {
  var kaydedilemeyen = [];
  for (var i = 0; i < IHTIDA_GORSEL_ADLARI.length; i++) {
    var tanim = IHTIDA_GORSEL_ADLARI[i];
    var veri = gorseller && gorseller[tanim.anahtar];
    if (!veri) continue;
    try {
      var blob = veriUrlBlob(veri, ref + tanim.sonEk);
      if (blob) klasor.createFile(blob);
      else kaydedilemeyen.push(tanim.anahtar);
    } catch (hata) {
      console.error("ihtida gorseli kaydedilemedi (" + tanim.anahtar + "): " + hata);
      kaydedilemeyen.push(tanim.anahtar);
    }
  }
  return kaydedilemeyen;
}

var KAYIT_GORSEL_ADLARI = [
  { anahtar: "on", sonEk: " - kimlik-on.jpg" },
...
function kayitImzaOku(klasor, ref, okunamayanlar) {
  ...
  try {
    ...
    while (it.hasNext()) {
      ...
      if (dosya.getSize() > 1024 * 1024) throw new Error("imza-cok-buyuk");
      var blob = dosya.getBlob();
      if (blob.getContentType() !== "image/png") throw new Error("imza-tur-gecersiz");
      return "data:image/png;base64," + Utilities.base64Encode(blob.getBytes());
    }
  } catch (hata) {
    if (okunamayanlar) okunamayanlar.push({ dosya: ad, neden: "imza-okunamadi" });
    console.error("Kayıt imzası okunamadı: " + ref);
  }
  return "";
}
...
function kayitGorselleriKaydet(klasor, ref, kimlik) {
  ...
  if (!kimlik || kimlik.yol !== "yukle") return sonuc;
  KAYIT_GORSEL_ADLARI.forEach(function (tanim) {
    var veri = kimlik[tanim.anahtar];
    if (!veri) return;
    try {
      var blob = veriUrlBlob(veri, ref + tanim.sonEk);
      if (!blob) throw new Error("Görsel çözülemedi");
      klasor.createFile(blob);
      sonuc[tanim.anahtar] = true;
    } catch (hata) {
      console.error("Kayıt kimlik görseli kaydedilemedi: " + tanim.anahtar);
      sonuc.hatalar.push(tanim.anahtar);
    }
  });
  return sonuc;
}

function kayitGorselleriOku(klasor, ref, okunamayanlar) {
  // 13 Eyl 2026: Tam adlı iki dosya okunur; MIME ve toplam JSON yanıtı 4 MiB ile sınırlıdır.
  var sonuc = { on: "", arka: "" }, toplam = 1024;
  KAYIT_GORSEL_ADLARI.forEach(function (tanim) {
    ...
    try {
      ...
      while (it.hasNext()) {
        var aday = it.next();
        if (aday.getName() === ad && !aday.isTrashed()) { dosya = aday; break; }
      }
      if (!dosya) return;
      if (dosya.getSize() > 4 * 1024 * 1024) { okunamayanlar.push({ dosya: ad, neden: "cok-buyuk" }); return; }
      var blob = dosya.getBlob(), bayt = blob.getBytes(), tur = blob.getContentType();
      if (!bayt || !bayt.length) { okunamayanlar.push({ dosya: ad, neden: "bos" }); return; }
      if (!/^image\/(jpeg|png|webp)$/.test(tur)) { okunamayanlar.push({ dosya: ad, neden: "tur-gecersiz" }); return; }
      var boyut = 4 * Math.ceil(bayt.length / 3) + tur.length + 32;
      if (toplam + boyut > 4 * 1024 * 1024) { okunamayanlar.push({ dosya: ad, neden: "cok-buyuk" }); return; }
      sonuc[tanim.anahtar] = "data:" + tur + ";base64," + Utilities.base64Encode(bayt);
      toplam += boyut;
    } catch (hata) { okunamayanlar.push({ dosya: ad, neden: "hata" }); }
  });
  sonuc.imza = kayitImzaOku(klasor, ref, okunamayanlar);
  return sonuc;
}

function kayitPdfGorselleriniEkle(veri, klasor, ref) {
  var okunamayan = [], g = kayitGorselleriOku(klasor, ref, okunamayan);
  if (okunamayan.length) throw new Error("kayit-gorseli-okunamadi");
  if (g.imza) veri.imza = g.imza;
  veri.kimlik = { yol: "yukle", on: g.on || "", arka: g.arka || "" };
}

function kayitBelgeIsle(e) {
  // 13 Eyl 2026: Panel anahtarı ve sıkı referans olmadan Drive okunmaz.
  try {
...
function kayitGorselleriniCopeAt(klasor, ref) {
  // 13 Eyl 2026: Tek geçerli referansın yalnız ada birebir eşleşen iki dosyasını çöpe atar.
  if (!/^UC-\d{4}-\d{4}$/.test(ref)) throw new Error("ref-gecersiz");
  var sonuc = { silinen: [], bulunamayan: [] };
  KAYIT_GORSEL_ADLARI.forEach(function (tanim) {
    var ad = ref + tanim.sonEk, it = klasor.getFilesByName(ad), bulundu = false;
    while (it.hasNext()) {
      var dosya = it.next();
      if (dosya.getName() !== ad) continue;
      dosya.setTrashed(true);
      sonuc.silinen.push(ad);
      bulundu = true;
    }
    if (!bulundu) sonuc.bulunamayan.push(ad);
  });
  return sonuc;
}

function kayitGorselSilIsle(e) {
  // 13 Eyl 2026: Yetkili panel çağrısı dışında silme yok; toplu arama kullanılmaz.
  try {
    if (!panelYetkiTamam(e)) return json({ ok: false, hata: "yetki" });
    var ref = String(e.parameter.ref || "");
    if (!/^UC-\d{4}-\d{4}$/.test(ref)) return json({ ok: false, hata: "ref-gecersiz" });
    var sonuc = kayitGorselleriniCopeAt(klasorGetir(), ref);
    // Kimlik PDF'ye de eklendiğinden rıza geri alındığında güncel arşiv PDF'i yenilenir.
    var yenile = kayitDuzeltIsle({ parameter: { anahtar: e.parameter.anahtar, ref: ref, pdf: "1" } });
    var yenileSonuc = JSON.parse(yenile.getContent());
    if (!yenileSonuc.ok) return json({ ok: false, hata: "kimlik-pdf-temizlenemedi", silinen: sonuc.silinen });
    return json({ ok: true, ref: ref, silinen: sonuc.silinen, bulunamayan: sonuc.bulunamayan });
  } catch (hata) { return json({ ok: false, hata: "gorsel-sil-hatasi" }); }
}

/** Panel için görselleri geri okur. 4 MB Apps Script'in yanıt boyutu korumasıdır. */
function ihtidaGorselleriOku(klasor, ref, okunamayanlar) {
  var sonuc = {};
  for (var i = 0; i < IHTIDA_GORSEL_ADLARI.length; i++) {
    var tanim = IHTIDA_GORSEL_ADLARI[i];
    var ad = ref + tanim.sonEk;
    ...
    try {
      ...
      sonuc[tanim.anahtar] = "data:" + blob.getContentType() + ";base64," + Utilities.base64Encode(bayt);
    } catch (hata) {
      console.error("ihtida gorseli okunamadi (" + ad + "): " + hata);
...
/** Bildirim e-postasının tek satırlık görsel özeti. */
function ihtidaGorselOzeti(gorseller, kaydedilemeyen) {
  var g = gorseller || {}, gelen = [];
  for (var i = 0; i < IHTIDA_GORSEL_ADLARI.length; i++) {
    var a = IHTIDA_GORSEL_ADLARI[i].anahtar;
    if (g[a]) gelen.push(a);
  }
  var metin = gelen.length ? gelen.join(", ") : "yok";
  if (kaydedilemeyen && kaydedilemeyen.length) metin += "  ⚠ KAYDEDİLEMEYEN: " + kaydedilemeyen.join(", ");
  return metin;
}

/** ?islem=ihtida-gorsel-sil&anahtar=…&ref=IH-2026-0007 — belge basıldıktan sonra kimlik
 *  görüntülerini kaldırmak için. YALNIZ bu referansın dosyalarını, ADI BİREBİR eşleşenleri,
 *  tek tek çöpe atar. Toplu silme yasaktır (25 Ağustos 2026 dersi). */
function ihtidaGorselSilIsle(e) {
  try {
    if (!panelYetkiTamam(e)) return json({ ok: false, hata: "yetki" });
    var ref = String(e.parameter.ref || "").trim();
    if (!/^IH-\d{4}-\d{4}$/.test(ref)) return json({ ok: false, hata: "ref-gecersiz" });
    var klasor = ihtidaKlasorGetir();
    var silinen = [], kalan = [];
    for (var i = 0; i < IHTIDA_GORSEL_ADLARI.length; i++) {
      var ad = ref + IHTIDA_GORSEL_ADLARI[i].sonEk;
      var it = klasor.getFilesByName(ad);
      var buldu = false;
      while (it.hasNext()) {
        var dosya = it.next();
        if (dosya.getName() !== ad) continue;                  // ada göre birebir eşleşme şartı
        dosya.setTrashed(true);
        silinen.push(ad);
        buldu = true;
      }
      if (!buldu) kalan.push(ad);
    }
    return json({ ok: true, surum: SURUM, ref: ref, silinen: silinen, bulunamayan: kalan });
  } catch (hata) {
    console.error(hata);
    return json({ ok: false, hata: "gorsel-sil-hatasi", ayrinti: String(hata).slice(0, 160) });
  }
}
...
function ihtidaPostIsleV2(v) {
  try {
    ...
    if (!anahtar) return json({ ok: false, hata: "gonderim-anahtari-gecersiz" });
    var onceki = null;

    var b = v.basvuran, onay = v.onay;
    var cami = ihtidaCamiCoz(v.cami);
    if (!cami) return json({ ok: false, hata: "cami-gecersiz" });
    v.cami = cami;
    var sahitler = Array.isArray(v.sahitler) ? v.sahitler : [];
    ...
    var kilit = LockService.getScriptLock();
    kilit.waitLock(30000);
    var ref, dosya, blob, gorselHatalari = [];
    try {
      ...
      // v23: EK-9 için gereken görseller ayrı dosyalar olarak saklanır (deftere sütun eklenmez).
      gorselHatalari = ihtidaGorselleriKaydet(klasor, ref, v.gorseller);
      ...
      satirEkle(sayfaV2, [
        ...
        (v.teslimat && v.teslimat.yontem) || "", v.sahitSecimi || "", onay.ek10 ? "Evet" : "Hayır",
        b.adresSokak || "", b.postaKodu || "", b.sehir || "", b.ulke || "", v.belgeTuru || "", "", "Otomatik PDF hazırlanıyor", "Sırada",
        cami.id, cami.ad, cami.sehir, cami.postaKodu, cami.adres, cami.kurum || "",
        onay.ek10Surumu, onay.imzaAktarimIzni ? "Evet" : "Hayır"
      ]);
      SpreadsheetApp.flush();
      ihtidaPaketKuyrugaAl(ref);
      if (typeof ihtidaDefteriKirlet === "function") ihtidaDefteriKirlet(ref);
      if (anahtar) ihtidaV2AnahtarKaydet(anahtar, ref);
    } finally {
      kilit.releaseLock();
    }

    // PDF üretimi kalıcı dakika tetikleyicisinde çalışır; form isteğini bekletmez.

    return json({ ok: true, ref: ref, tekrar: false, paket: "sirada" });
  } catch (hata) {
    console.error("ihtida-basvuru-hatasi");
    return json({ ok: false, hata: "sunucu-hatasi" });
  }
}

function ihtidaKopyaGonderV2(blob, ref, adSoyad, eposta, dil) {
  var adres = String(eposta || "").trim();
  if (!adres || !epostaGecerli(adres)) return;
  veliEpostaBaglami(dil, {kurum: "cami"}); // Dil verilmeden iki dilli/varsayılan bir ileti oluşturulmaz.
  var konu = {
  ...
  var metinler = {
    tr: [adSoyad + " adına yaptığınız İhtida Belgesi ön başvurusu alınmıştır. Referans numaranız: " + ref,
      "Doldurduğunuz form bu e-postanın ekindedir; lütfen saklayınız.",
      "Bu bir ön başvurudur. Resmî İhtida Belgesi (EK-9), camimizdeki törenin ardından T.C. Brüksel Büyükelçiliği Sosyal İşler Müşavirliği onayıyla düzenlenir.",
      "Tören tarihini belirlemek için camimiz sizinle ayrıca iletişime geçecektir."],
    fr: ["Votre pré-demande d’attestation de conversion au nom de " + adSoyad + " a bien été reçue. Numéro de référence : " + ref,
      "Le formulaire complété se trouve en pièce jointe ; conservez-le.",
      "Il s’agit d’une pré-demande. L’attestation officielle de conversion (EK-9) est délivrée après la cérémonie à la mosquée, avec la validation du Conseiller des Affaires sociales de l’Ambassade de Turquie à Bruxelles.",
      "La mosquée vous recontactera pour fixer une date."],
  ...
  try {
    epostaGonder({ to: adres, subject: konu, body: govde + "\n\n" + KIMLIK.yazisma.imza.cami[dil].join("\n"),
      htmlBody: mailHtml(govde, dil, konu, "cami"), attachments: [blob], kurum: "cami", name: KIMLIK.kurumlar.cami.gonderenAdi, replyTo: EPOSTA.yanit });
  } catch (hata) {
...
function panelBelgeIsle(e) {
  try {
    ...
    /* v23: EK-9'a basılacak vesikalık ile başvuranın imzası buradan gelir; kimlik görüntüleri
       resmî dosya içindir, panel bunları yalnız GÖSTERİR/indirir. Şahit imzaları kâğıttadır. */
    var okunamayanlar = [];
    var gorseller = ihtidaGorselleriOku(ihtidaKlasorGetir(), ref, okunamayanlar);

    return json({
      ok: true, surum: SURUM, ref: ref, kayit: kayit,
      gorseller: gorseller,
      vesikalik: gorseller.vesikalik,               // panel.js'in beklediği düz alanlar
      basvuranImza: gorseller.imza,
...
/* v21 — panel «Form PDF»: Drive'daki form dosyasını panel anahtarıyla base64 olarak verir.
   Neden: defterdeki «PDF bağlantısı» Drive'ın yalnız dosya SAHİBİNE (dernek hesabı) açık
   adresiydi. Panele başka bir Google hesabıyla girmiş yönetici her tıklamada Drive'ın
   «erişim isteyin» sayfasına düşüyordu (Rıdvan, 4 Eylül 2026; anonim istek 401). Dosyalar
   bağlantı paylaşımına AÇILMADI — çocuk/veli verisi «bağlantıyı bilen herkes»e değil, yalnız
   panel anahtarına açılır. Panel dosyayı buradan alır ve tarayıcıda kendisi açar.
   Güvenlik sınırı: yalnız kayıt ve ihtida klasörlerinin İÇİNDEKİ PDF'ler verilir; betiğin
   sahibi hesabın Drive'ındaki başka hiçbir dosya, kimliği bilinse bile bu uçtan çıkmaz.
   Çöpteki dosya da verilmez (eski-kimlik-temizle / arsiv-saglik-gizle eski kopyaları çöpe atar;
   defter hep yeni kopyayı gösterir, çöptekine ulaşan bir bağlantı kalmamalı). */
function panelPdfIsle(e) {
  try {
    if (!panelYetkiTamam(e)) return json({ ok: false, hata: "yetki" });
    var id = String(e.parameter.id || "").trim();
    if (!/^[A-Za-z0-9_-]{10,}$/.test(id)) return json({ ok: false, hata: "id-gecersiz" });

    var dosya;
    try { dosya = DriveApp.getFileById(id); } catch (yok) { return json({ ok: false, hata: "bulunamadi" }); }
    if (dosya.isTrashed()) return json({ ok: false, hata: "bulunamadi" });

    var izinli = [klasorGetir().getId(), ihtidaKlasorGetir().getId()];
    var ebeveynler = dosya.getParents(), klasorde = false;
    while (ebeveynler.hasNext()) {
      if (izinli.indexOf(ebeveynler.next().getId()) !== -1) { klasorde = true; break; }
    }
    if (!klasorde) return json({ ok: false, hata: "bulunamadi" });
    if (dosya.getMimeType() !== MimeType.PDF) return json({ ok: false, hata: "pdf-degil" });

    var bayt = dosya.getBlob().getBytes();
    return json({ ok: true, surum: SURUM, ad: dosya.getName(), boyut: bayt.length, pdfB64: Utilities.base64Encode(bayt) });
  } catch (hata) {
    console.error(hata);
    return json({ ok: false, hata: "pdf-hatasi", ayrinti: String(hata).slice(0, 160) });
  }
}
...
function kayitDuzeltIsle(e) {
  ...
  try {
    ...
    if (degisen.length || pdfIste) {
      ...
      kayitPdfGorselleriniEkle(veri, klasor, ref);
...
/* brevoIzin: script.external_request (UrlFetchApp) iznini elden tetiklemek icin — editorden bir kez calistirilir; web app (doGet/doPost) BU fonksiyonu cagirmaz. */
function brevoIzin() {
