...
| Dosya | Durum |
|---|---|
| `ulucamii-Kod-v40.gs` | **Canlıdaki sürüm — 23 Eylül 2026, 14:45.** Herkese açık `kayit`/`ihtida` POST uçlarına sunucu tarafı hacim sınırı (`AYAR_BASVURU_SINIRI`: tür başına 10 dakikada 10 yeni kayıt → `cok-sik`; günde kayıt 60 / ihtida 20 → `gunluk-sinir`; aynı e-posta günde 3 → `eposta-gunluk-sinir`; sayım defterdeki gerçek satırlardan, kilit içinde, tekrar denetiminden sonra — aynı `gonderimAnahtari` hiç engellenmez) + tuzak alan `web` doluysa `bos-istek`; sağlıkta `basvuruSiniri: true`. Dağıtım `D:/tmp/gas/dagit_v40.py`. v39 — 21 Eylül 2026, 14:09: site dilleri nl/de (`SITE_DILLERI`, beş dilli sözlükler; tr/fr/en çıktıları birebir), seviye testi form sürümü 2 (yer bilgisi + iki isteğe bağlı paylaşım onayı, deftere sona 8 sütun). v38 — 20 Eylül 2026: seviye tespit testi (`tur:"seviye"`, `seviye-detay`, `seviye-sil`), `olcek` e-posta bloğu. Ayrıntı: [Seviye testi](../../docs/SEVIYE-TESTI.md), [diller](../../docs/DIL-NL-DE.md). |
...
| `veli-eposta-sablon.gs`, `veli-cuma.gs`, `veli-mail-listesi.gs`, `ihtida-*.gs`, `seviye-testi-isleri.gs` | Ortak modüller; `npm run ihtida:gas-derle` hepsini `.codex/cikti/gas/ulucamii-v38.gs` paketine birleştirir (sürümle birlikte ad değişir). Derleme ayrıca `src/lib/seviye-testi/gas-giris.ts`'i `SeviyeTesti` IIFE paketi olarak ekler — soru bankası ve puanlama sunucuya tek kaynaktan gider. |
...
| | |
|---|---|
| `ulucamii-Kod-v40.gs` | Depodaki ana kaynak ve canlıdaki sürüm (v40 — 23 Eylül 2026: kayıt/ihtida hacim sınırı + tuzak alan; v39 — 21 Eylül 2026: site dilleri nl/de + seviye testi form sürümü 2; v38 — 20 Eylül 2026: seviye tespit testi). v37 — 15 Eylül 2026: zorunlu çizilen veli imzası ve kimlik yükleme; kimlik PDF eki. Ayrıntı: [Kayıt belgeleri](../../docs/KAYIT-ZORUNLU-BELGELER.md). |
