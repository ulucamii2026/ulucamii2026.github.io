/* v25 — Otomatik tam PDF, özel arşiv, üç alıcı ve teslim takibi. */
/**
 * Marche-en-Famenne Ulu Camii — Ortak Apps Script Alıcı (SÜRÜM 23)
...
function doGet(e) {
  ...
  return json({ ok: true, servis: "ulucamii-alici", surum: SURUM, ihtidaPaketHazir: typeof IhtidaPdf !== "undefined" && PropertiesService.getScriptProperties().getProperty("IHTIDA_PAKET_KURULU") === "25", zaman: new Date().toISOString() });
}
...
function doPost(e) {
  try {
    ...
    if (v.tur === "ihtida") {
      if (typeof IhtidaPdf === "undefined" || PropertiesService.getScriptProperties().getProperty("IHTIDA_PAKET_KURULU") !== "25") return json({ ok: false, hata: "ihtida-paket-hazir-degil" });
...
function ihtidaTeslimMetni(v, dil) {
  ...
  if (secim === "cami") return dil === "fr" ? "Envoi à la mosquée, retrait sur place" : dil === "en" ? "Post to the mosque for collection" : "Camiye postalanacak, camiden teslim alınacak";
  var b = v.basvuran || {};
