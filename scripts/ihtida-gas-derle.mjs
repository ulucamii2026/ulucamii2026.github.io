...
const parts = await Promise.all(['scripts/apps-script/kimlik-sabitler.gs', 'scripts/apps-script/ulucamii-Kod-v31.gs', 'scripts/apps-script/ihtida-paket-isleri.gs', 'scripts/apps-script/ihtida-defteri-isleri.gs', 'scripts/apps-script/veli-mail-listesi.gs', 'scripts/apps-script/veli-eposta-sablon.gs', 'scripts/apps-script/veli-cuma.gs', 'public/vendor/pdf-lib.min.js', 'public/vendor/fontkit.umd.min.js'].map(oku));
const output = new URL('.codex/cikti/gas/ulucamii-v31.gs', root);
await mkdir(new URL('.', output), { recursive: true });
await writeFile(output, parts.join('\n;\n') + '\n;\n' + built.outputFiles[0].text + '\n;\n' + defter.outputFiles[0].text + '\nvar IHTIDA_PDF_KAYNAKLARI = ' + JSON.stringify(encoded) + ';\n');
console.log('GAS v31 derlendi; kimlik sabitleri, kod, açık şablonlar ve fontlar.');
