...
const parts = await Promise.all(['scripts/apps-script/ulucamii-Kod-v25.gs', 'scripts/apps-script/ihtida-paket-isleri.gs', 'public/vendor/pdf-lib.min.js', 'public/vendor/fontkit.umd.min.js'].map(oku));
const output = new URL('.codex/cikti/gas/ulucamii-v25.gs', root);
await mkdir(new URL('.', output), { recursive: true });
await writeFile(output, parts.join('\n;\n') + '\n;\n' + built.outputFiles[0].text + '\nvar IHTIDA_PDF_KAYNAKLARI = ' + JSON.stringify(encoded) + ';\n');
console.log('GAS v25 derlendi; yalnız kod, açık şablonlar ve fontlar.');
