...
const source=['kimlik-sabitler.gs','veli-eposta-sablon.gs','ulucamii-Kod-v32.gs'].map(ad=>readFileSync(new URL('../scripts/apps-script/'+ad,import.meta.url),'utf8')).join('\n');
...
test('v29: yetkisiz/bozuk referanslı yeni uçlar Drive’a erişemez', () => {
 ...
 c.VELI_PORTAL_SURUM = 'test'; assert.equal(c.doGet({}).kayitKimlik, true); assert.equal(c.doGet({}).surum, 32);
});
