...
function runtime(){const props={};let sends=0,found='';const x=vm.createContext({console:{...console,warn(){}},PropertiesService:{getScriptProperties:()=>({getProperty:k=>props[k]??null,setProperty:(k,v)=>props[k]=v,deleteProperty:k=>{delete props[k];},getProperties:()=>({...props})})},LockService:{getScriptLock:()=>({tryLock:()=>true,releaseLock(){}})},Utilities:{getUuid:()=> 'e3d7c872-0f84-4d8a-8a50-a098c15fdf2b',formatDate:()=>'2026-09-14'}});vm.runInContext(source,x);Object.assign(x,{veliPortalKimlikDogrula(){},veliPortalHash:s=>s,brevoAnahtari:()=> 'fixture',veliCumaHazirla:()=>({alicilar:[{eposta:'veli@example.test',dil:'fr'}],icerik:{fr:{}}}),veliCumaGonder:()=>{sends++;return {durum:'saglayici-kabul',messageId:'example'};},veliCumaBelirsizYokla:()=>found});return {x,props,sends:()=>sends,setFound:v=>found=v};}
...
test('v36: gönderim durumları cuma başına tek özellikte; eski tekil kayıtlar ve çeviri önbelleği bakımla katlanır/silinir',()=>{
  const r=runtime();
  r.props['VELI_CUMA_GONDERIM_2026-09-11_veli@example.test']=JSON.stringify({durum:'saglayici-kabul',messageId:'eski'});
  r.props['VELI_CUMA_GONDERIM_2026-06-05_x@example.test']=JSON.stringify({durum:'saglayici-kabul'});
  r.props['VELI_CUMA_FR_abc']='Les lettres';r.props['BREVO_API_KEY']='fixture';
  const s=r.x.veliCumaIsle('2026-09-11',{});
  assert.equal(r.sends(),0,'eski tekil kayıt tanındı, yeniden gönderilmedi');assert.equal(s.onceki,1);
  assert.deepEqual({...s.bakim},{katlanan:1,silinen:2,toplam:3});
  assert.deepEqual(Object.keys(r.props).sort(),['BREVO_API_KEY','VELI_CUMA_GONDERIM_2026-09-11','VELI_CUMA_SONUC']);
  assert.equal(JSON.parse(r.props['VELI_CUMA_GONDERIM_2026-09-11'])['veli@example.test'].messageId,'eski');
  r.x.veliCumaIsle('2026-09-18',{});assert.equal(r.sends(),1);
  const m=JSON.parse(r.props['VELI_CUMA_GONDERIM_2026-09-18']);assert.equal(m['veli@example.test'].durum,'saglayici-kabul');assert.equal(m['veli@example.test'].anahtar,'e3d7c872-0f84-4d8a-8a50-a098c15fdf2b');
  assert.equal(Object.keys(r.props).filter(k=>k.startsWith('VELI_CUMA_GONDERIM_')).length,2);
  r.props['VELI_CUMA_GONDERIM_2026-09-25']='[1,2';assert.deepEqual({...r.x.veliCumaDurumlar(r.x.PropertiesService.getScriptProperties(),'2026-09-25')},{});
});
test("v36: çeviri önbelleği CacheService'te (6 saat), Script Property yazılmaz",()=>{
  const cache={};let ceviri=0;const x=vm.createContext({console,CacheService:{getScriptCache:()=>({get:k=>cache[k]??null,put:(k,v,s)=>{cache[k]=v;cache._sure=s;}})},LanguageApp:{translate:()=>{ceviri++;return 'Les lettres';}},PropertiesService:{getScriptProperties:()=>{throw new Error('property yazılmamalı');}}});
  vm.runInContext(source,x);x.veliPortalHash=s=>s;
  assert.equal(x.veliCumaCevir('Harfler'),'Les lettres');assert.equal(x.veliCumaCevir('Harfler'),'Les lettres');assert.equal(ceviri,1);assert.equal(cache._sure,21600);
});
