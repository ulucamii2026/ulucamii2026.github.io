...
- Yönetim (Claude): `~/.claude/skills/ulucamii-site/scripts/portal-yonetim.py` — `ice-aktar` (kayıt defteri → Firestore), `hoca-ekle`, `ayar`, `listele`, `sil`. Hoca ekranında da «Kayıt defterinden yenile» ve «Davet gönder» var. Mükerrer/deneme kayıtlar `ayarlar/portal.atlanan` listesiyle atlanır; `adSabit=true` olan öğrencinin adı defterden ezilmez; bir öğrencide birden çok veli e-postası olabilir. Davetler: `portal-yonetim.py davet <e-posta|hepsi> [--hoca]` (info@ulucamii.be üzerinden; Firebase'in ücretsiz planı günde 5 giriş-bağlantısı e-postasıyla sınırlı).
